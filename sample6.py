...
def main():
    ...
    print(
        "  Click a masked face -> reveal / mask toggle"
    )
    print(
        "  R             -> reset identities"
    )
    ...
    try:
        ...
        while True:
            ...
            for track in (
                ...
                # Match manually revealed people by embedding so a
                # reveal survives track deletion and re-entry.
                persistent_manual_reveal = False

                if not track.whitelisted:
                    for saved_embedding in enrollment.manual_revealed_embeddings:
                        if cosine_similarity(
                            track.embedding, saved_embedding
                        ) >= REID_THRESHOLD:
                            persistent_manual_reveal = True
                            break

                # Fail-safe still wins when AI is stale. Otherwise, a
                # manually revealed bystander remains visible.
                if (
                    failsafe_active
                    or
                    (
